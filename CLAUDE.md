...
| 구성 요소 | 위치 | 역할 |
|-----------|------|------|
| 클러스터 드롭다운 | 검색 조건 2행 | 선택 후 검색 시 API에 `clusters=` 전달 (서버 필터링) |
| 클러스터 탭 | 결과 상단 | 클릭 시 클라이언트 필터링 (API 재호출 없음), 검색 후 드롭다운 선택에 맞는 탭 자동 활성화 |

탭 우측에 `cluster1: 37건 / 전체 142건` 형식으로 현재 표시 건수 출력.
