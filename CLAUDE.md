...
### 커서 페이지네이션 (`cm_client.py:fetch_all_clusters_stream`)

CM impalaQueries API는 filter 포함 시 내부 scan limit에 걸려 결과가 극히 적음.
조건이 있는 검색은 아래 방식으로 우회:

1. 시간 범위를 `CURSOR_CHUNK_HOURS`(10분) 단위 청크로 분할
2. 각 청크를 **filter 없이** `limit=CURSOR_CHUNK_LIMIT(1000)`으로 CM 요청
3. 수신된 결과에 Python 사이드 필터(`_matches_conditions`) 적용
4. `seen_ids` set으로 청크 간 중복 제거
5. 진행 중 SSE progress 이벤트(`new_queries` 포함)로 클라이언트에 스트리밍

조건이 없는 경우: 청크 분할 없이 단일 요청 (`limit=1000`, 시간 범위 파라미터 포함).
...
**2행**: 클러스터 드롭다운 / Query Type / 검색·초기화 버튼
...
- CM API `limit` 파라미터 미전송 시 기본값 100 적용됨 → `CURSOR_CHUNK_LIMIT=1000`을 항상 명시 전송
- **결과 수 제한 없음**: 검색 조건에 해당하는 모든 쿼리를 반환 (UI에서 결과 수 선택 제거됨)
- CM impalaQueries에 filter 포함 시 "impala query scan limit reached"로 결과가 극히 적음 → 커서 페이지네이션에서 CM에 filter 전송하지 않고 Python 사이드에서 필터링
